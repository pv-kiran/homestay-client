...
const schema = yup.object().shape({
    homelyFoodCenterName: yup.string().required('HomelyFood center name is required'),
...
export default function HomelyFood() {
    ...
    } = useApi(adminService.adminHomelyFoodAdd);
    ...
    } = useApi(adminService.adminHomelyFoodEdit);
    ...
    const onSubmit = async (data) => {
        ...
        const { city, menuItems, homelyFoodCenterName } = data;
        const filteredData = { city, menuItems, homelyFoodCenterName };
    ...
    return (
        <>
            <div className="flex justify-end">
                <Button onClick={() => setIsModalOpen(true)} size="sm"><CirclePlus className='pr-1 pb-1' color="#ffffff" />Add Homely food</Button>
                <Modal
                    isOpen={isModalOpen}
                    onClose={handleClose}
                    title={"Add Homely food"}
                    description={"Homely food details"}
                    maxWidth="600px"
                >
                    <form onSubmit={handleSubmit(onSubmit)} className="bg-white rounded-lg p-6 space-y-6">
                        ...
                        <div>
                            <label
                                htmlFor="homelyFoodCenterName"
                                className="block text-sm font-medium text-gray-700"
                            >
                                Homely food center Name
                            </label>
                            <input
                                {...register('homelyFoodCenterName')}
                                type="text"
                                id="homelyFoodCenterName"
                                ...
                                placeholder="Enter Homely food center name"
                            />
                            {errors.homelyFoodCenterName && (
                                <p className="mt-1 text-sm text-red-600">{errors.homelyFoodCenterName.message}</p>
                            )}
                        </div>
                        ...
                        <div>
                            {
                                !restaurentsLoading && <EmptyState
                                    title="Empty Homely food"
                                    message="Your Homely food list is currently empty."
